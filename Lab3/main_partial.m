...
clc
 ...
 global velocity tstop delta mass L b C12 C34 f Jz
...
% % Change "time" to "tstop" for variable time step.
 [timeout, xout]=ode45('bicycle_model',time,x0);
% % [timeout, xout]=ode45('bicycle_model',tstop,x0);
%%
...
%-------------------------------------------------------
vx=velocity;
vy_ss=delta*(velocity*(b*L*C12*C34-f*C12*mass*(velocity^2)))/((L^2)*C12*C34+(mass*velocity^2*(b*C34-f*C12)));
psi_p_ss=delta*(velocity*L*C12*C34)/((L^2)*C12*C34+(mass*velocity^2*(b*C34-f*C12)));
